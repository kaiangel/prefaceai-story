...
5. **集成可控**: PR 是唯一合并点，Code Review 保证质量
...
**实际需要做的**:
1. 创建 `CODEX.md` — Ben 的 Codex 启动时读取的上下文文件（浓缩项目关键信息）
2. 创建 `codex-agents/` 目录 — 存放 Ben 的 4 个 Agent instruction 文件
...
```
main (受保护，不允许直接 push)
  ├── founder/* (Founder 的 Agent 团队的分支)
  └── ben/* (Ben 的分支)
```

**PR 流程**:
- 两边各自在自己的分支命名空间工作
- PR 合并到 main 需要对方 review 或至少知晓
- CI 基础检查（Python syntax、build test）

**执行**: DevOps agent 配置 GitHub branch protection + PR template
...
|-------|------|-----------------|------|
| Backend-B | 后端开发 | `codex-agents/backend-b.md` | API 开发、服务实现 |
| DB-Expert | 数据库专家 | `codex-agents/db-expert.md` | Schema 设计、迁移、查询优化 |
| Tester-B | 测试工程 | `codex-agents/tester-b.md` | Ben 负责模块的测试 |
| DevOps-B | 运维 | `codex-agents/devops-b.md` | Ben 负责模块的部署、CI/CD |
...
| # | 事项 | 负责 | 工作量 | 状态 |
|---|------|------|--------|------|
| 1 | 创建 `CODEX.md` | Coordinator/PM agent | ~30 分钟 | ⏳ 等 Founder 和 Ben 沟通后 |
| 2 | 创建 `codex-agents/` 目录 + 4 个 instruction 文件 | Coordinator/PM agent | ~30 分钟 | ⏳ |
| 3 | GitHub branch protection 设置 | DevOps agent | ~15 分钟 | ⏳ |
...
| # | 事项 | 负责 | 时机 |
|---|------|------|------|
| 6 | Ben 读完 CODEX.md + 阅读清单 | Ben | 第一天 |
...
5. **Git 工作流**: Founder 对分支保护技术不太熟，但认同有必要就做
