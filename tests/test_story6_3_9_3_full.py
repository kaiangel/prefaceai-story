#!/usr/bin/env python3
"""
teststory6.3.9.3 - 验证修复后的端到端测试

修复内容：
1. 名字重复bug（Lin Xiaoyu Lin Xiaoyu -> Lin Xiaoyu）
2. brief_identifier准确性（包含眼镜、条纹等关键特征）
3. 测试前清理输出目录
"""
...
# 输出目录
OUTPUT_DIR = Path("test_output/manualtest/teststory6.3.9.3")
...
async def run_full_test():
    ...
    print("=" * 80)
    print("teststory6.3.9.3 - 验证修复后的端到端测试")
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("teststory6.3.9.3 - 验证修复后的端到端测试\n")
        ...
        f.write("## 角色列表（验证brief_identifier准确性）\n")
        for char in characters:
            char_id = char.get('id')
            char_name = char.get('name')
        ...
        f.write("\n## prompt_log.json中的char_count验证\n")
        for entry in debug_logger.prompt_log:
            f.write(f"Shot {entry['shot_id']}: char_count={entry['char_count']}\n")
    ...
    for i in range(max_shots):
        ...
        if prompt_file.exists():
            ...
            for char in characters:
                ...
                if name_en:
                    # 检查 "Name Name" 或 "Name's Name"
                    if f"{name_en} {name_en}" in content or f"{name_en}'s {name_en}" in content:
                        print(f"   Shot {i+1}: 发现名字重复 '{name_en}'")
                        name_repeat_found = True
    if not name_repeat_found:
        print("   没有发现名字重复问题")

    # 验证2：brief_identifier准确性
    print("\n2. brief_identifier验证:")
    for char in characters:
        char_id = char.get('id')
        char_name = char.get('name')
        brief_id = storyboard._build_brief_identifier(char)
        ...
        issues = []
        # 检查眼镜
        has_glasses_in_data = any('glass' in a.lower() for a in accessories)
        has_glasses_in_brief = 'glasses' in brief_id.lower()
        if has_glasses_in_data and not has_glasses_in_brief:
            issues.append("缺少眼镜")

        # 检查条纹
        has_stripe_in_data = 'stripe' in top.lower()
        has_stripe_in_brief = 'stripe' in brief_id.lower()
        if has_stripe_in_data and not has_stripe_in_brief:
            issues.append("缺少条纹")

        # 检查医疗服
        has_scrub_in_data = 'scrub' in top.lower() or 'medical' in top.lower()
        has_scrub_in_brief = 'scrub' in brief_id.lower() or 'medical' in brief_id.lower()
        if has_scrub_in_data and not has_scrub_in_brief:
            issues.append("缺少医疗服")

        if issues:
            print(f"   {char_name}: {brief_id} - 问题: {', '.join(issues)}")
        else:
            print(f"   {char_name}: {brief_id}")
    ...
    for entry in debug_logger.reference_log:
        ...
        # 每个角色2张参考图，所以char_refs应该是角色数*2
        expected_chars = char_refs // 2
        sources = entry['char_ref_sources']
        print(f"   Shot {shot_id}: {expected_chars}个角色的参考图 ({char_refs}张)")
