#!/usr/bin/env python3
"""
teststory6.3.9.2 - 全新故事端到端测试

验证修复后的参考图传递和新prompt策略
"""
...
# 输出目录
OUTPUT_DIR = Path("test_output/manualtest/teststory6.3.9.2")
...
async def run_full_test():
    ...
    print("=" * 80)
    print("teststory6.3.9.2 - 全新故事端到端测试")
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("teststory6.3.9.2 - 全新故事端到端测试\n")
        ...
        for entry in debug_logger.reference_log:
            ...
            f.write(f"Shot {shot_id}: {total}张参考图 (角色:{char_count}, 场景:{scene_count})\n")
    ...
    for entry in debug_logger.reference_log:
        if entry['total_refs'] == 0:
            print(f"   ❌ Shot {entry['shot_id']}: 参考图数量为0!")
            all_ok = False
        else:
