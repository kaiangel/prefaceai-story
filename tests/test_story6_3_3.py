#!/usr/bin/env python3
"""
teststory6.3.3 - 验证P0-1/P0-2/P0-3修复效果

新增修复内容（相比6.3.2）：
- P0-1: story_generation.py添加视觉一致性约束（角色服装必须引用定义）
- P0-2: _infer_location_type修复（优先检查外景关键词）
- P0-3: 删除view_types=['interior']约束（让系统自动推断）

验证重点：
1. story.json中张野的服装是否全程一致（黑色V领毛衣，不是burgundy shirt）
2. scene_refs是否包含exterior图片（"便利店外观"应生成exterior）
3. shot_01 vs shot_02: 便利店外观是否一致？（使用exterior scene_ref）
4. shot_13: 张野是否穿黑色V领毛衣（而非酒红色衬衫）

输出目录: test_output/manualtest/teststory6.3.3/
"""
...
# 输出目录
OUTPUT_DIR = Path("test_output/manualtest/teststory6.3.3")
