#!/usr/bin/env python3
"""
teststory6.3.4 - 验证P0-4修复效果（服装描述清洗）

新增修复内容（相比6.3.3）：
- P0-4: _clean_visual_description()增强
  - 移除visual_description中的服装颜色+款式描述
  - 避免与CHARACTERS部分的正确服装描述冲突
  - 例如："in a burgundy silk shirt" → 被清洗掉
  - 例如："in white scrubs" → 被清洗掉

验证重点：
1. 角色外观一致性：
   - char_001（陈默/女护士）：高马尾+刘海、粉色护理夹克、手表
   - char_002（李想/程序员）：黑框眼镜必须保持、灰色卫衣
   - char_003（张野/调酒师）：黑色V领毛衣（不是burgundy shirt）
2. 眼镜不消失：char_002在所有shot中都应戴眼镜
3. 刘海不消失：char_001在所有shot中都应有刘海
4. 背景人物不变形：char_002/char_003作为背景时不应变形

输出目录: test_output/manualtest/teststory6.3.4/
"""
...
# 输出目录
OUTPUT_DIR = Path("test_output/manualtest/teststory6.3.4")
...
async def run_full_e2e_test():
    ...
    print("="*80)
    print("teststory6.3.4 - 验证P0-4修复效果（服装描述清洗）")
    print("="*80)
    print("\n新增修复内容（P0-4）：")
    print("  - _clean_visual_description()增强")
    print("  - 移除visual_description中的服装颜色+款式描述")
    print("  - 例如：'in a burgundy silk shirt' → 被清洗掉")
    print("  - 避免与CHARACTERS部分的正确服装描述冲突")
    print("\n验证重点：")
    print("  1. 角色外观一致性：char_001刘海, char_002眼镜, char_003黑V领")
    print("  2. 眼镜不消失：char_002（程序员）在所有shot中都应戴眼镜")
    print("  3. 刘海不消失：char_001（女护士）在所有shot中都应有刘海")
    print("  4. 背景人物不变形")
    print("="*80)
    ...
    results = {
        "test_name": "teststory6.3.4",
        "description": "验证P0-4修复效果（服装描述清洗）",
        "fixes_tested": [
            "P0-4: _clean_visual_description()增强，移除服装颜色+款式描述",
            "避免visual_description中的'in a burgundy silk shirt'与CHARACTERS部分冲突"
        ],
        "steps": {},
        "verification": {}
    }
