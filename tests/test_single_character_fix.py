...
OUTPUT_DIR = "test_output/manualtest/single_char_fix_test"
...
async def test_style_enforcer():
    """测试风格强制器"""
    print("\n" + "=" * 60)
    print("测试1: 风格强制器 (StyleEnforcer)")
    print("=" * 60)

    test_prompt = "A young man standing in a convenience store"

    print("\n原始prompt:")
    print(test_prompt)

    print("\n应用风格强制后 (realistic):")
    enforced = StyleEnforcer.enforce_prompt(test_prompt, "realistic")
    print(enforced[:500] + "..." if len(enforced) > 500 else enforced)

    print("\n风格负面提示词:")
    negative = StyleEnforcer.build_style_negative_prompt("realistic")
    print(negative)
...
async def main():
    ...
    try:
        ...
        if refs_ok:
            # 测试3: 场景图生成
            await asyncio.sleep(3)  # 等待一下避免速率限制
        ...
        if refs_ok and scene_ok:
            ...
            print(f"  2. {OUTPUT_DIR}/character_refs/char_001_fullbody.png - 全身参考图（应与肖像是同一人）")
            print(f"  3. {OUTPUT_DIR}/images/scene_01.png - 场景图（角色应与参考图一致）")
            print(f"\n关键验证点:")
            print(f"  - 所有图片都应该是写实摄影风格（非卡通/插画）")
            print(f"  - 肖像和全身图应该是同一个人")
            print(f"  - 场景图中的角色应该与参考图一致")
        else:
            print(f"\n❌ 部分测试失败，请检查错误信息")
